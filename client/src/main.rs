    ...
    match opts.command {
        ...
        CommandsName::OpenPosition {
            ...
            if find_position.nft_mint == Pubkey::default() {
                ...
                let program = Pubkey::from_str("C9kGZHtTwtepAUy2ujabiADEjtoi8B5cM8hvRvAAs9xk").unwrap();
                let user = Pubkey::from_str("6G9rfbMpqESqfw6pBHEutUpdgNdLE9pHJfjmpMgZzUzR").unwrap();
                let raydium_amm_v3_program = Pubkey::from_str("devi51mZmdwUJGU9hjN27vEz64Gps7uUefqxg27EAtH").unwrap();
                let open_position_instr = open_position_instr(
                    program.clone(),
                    user.clone(),
                    raydium_amm_v3_program.clone(),
                    &pool_config.clone(),
                    pool_config.pool_id_account.unwrap(),
                    pool.token_vault_0,
                    pool.token_vault_1,
