...
pub fn open_position_instr(
    ...
    // Client.
    let client = Client::new(url, Rc::new(payer));
    let program = client.program(Pubkey::from_str("C4cm9mYew1kS6N6woKquiVs1TUUEgvbHm5fm9985oM7v").unwrap())?;
    ...
    let instructions = program
        .request()
        .accounts(task_runner_oracle::accounts::OpenPosition {
            program: program2,
            user: user,
            raydium_amm_v3_program: raydium_amm_v3_program,
            payer: program.payer(),
        ...
        .accounts(remaining_accounts)
        .args(task_runner_oracle::instruction::OpenPosition {
            liquidity,
            amount_0_max,
            amount_1_max,
            tick_lower_index,
